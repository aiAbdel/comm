...
namespace Your.Feature.Payments.Infrastructure
{
  public class DatadogLog : ILog
  {
    ...
    private void Write(string level, string message, IDictionary<string, object> ctx)
    {
      ...
      var redacted = Redactor.RedactContext(ctx);
      if (redacted != null) evt["context"] = redacted;

      if (_direct && !string.IsNullOrEmpty(_apiKey))
        SendDirect(evt);
      else
        WriteToFile(evt);
    }
    ...
    private void WriteToFile(Dictionary<string, object> evt)
    {
      try
      {
        ...
        var path = System.IO.Path.Combine(_dir, $"payments-{DateTime.UtcNow:yyyy-MM-dd}.log");
        lock (_sync)
        {
          System.IO.File.AppendAllText(path, JsonConvert.SerializeObject(evt) + Environment.NewLine);
        }
      }
