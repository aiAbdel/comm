...
namespace Your.Feature.Payments.Services
{
  public class ZuoraAuth : IZuoraAuth
  {
    ...
    public async Task<string> GetTokenAsync()
    {
      ...
      try {
        var resp = await $"{_baseUrl}/oauth/token".PostUrlEncodedAsync(new { client_id = _clientId, client_secret = _clientSecret, grant_type = "client_credentials" }).ReceiveJson<dynamic>();
        string token = resp.access_token; int expires = resp.expires_in;
        ...
        _log.Error("Zuora OAuth token failed", ex, new System.Collections.Generic.Dictionary<string, object>{{"response",body}});
        throw;
      }
    }
  }
