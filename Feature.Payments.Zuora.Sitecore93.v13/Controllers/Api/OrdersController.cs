...
namespace Your.Feature.Payments.Controllers.Api
{
  public class OrdersController : Controller
  {
    ...
    public async System.Threading.Tasks.Task<ActionResult> Place(PlaceOrderDto d)
    {
      ...
      try {
        var idem = await _idem.GetOrCreateKeyAsync(d.AccountNumber, d.ProductRatePlanId, d.ProductRatePlanChargeId, d.Quantity);
        var res = await _zuora.CreateOrderAsync(order, idem);
        return Json(res);
      }
      catch (Exception ex) {
        _log.Error("Order placement failed", ex, new System.Collections.Generic.Dictionary<string, object>{{"accountNumber", d.AccountNumber},{"collect", d.Collect}});
        return new HttpStatusCodeResult(502, "Order placement failed");
      }
    }
    ...
    {
      if (d == null || string.IsNullOrWhiteSpace(d.AccountNumber))
        return new HttpStatusCodeResult(400, "Invalid request");
