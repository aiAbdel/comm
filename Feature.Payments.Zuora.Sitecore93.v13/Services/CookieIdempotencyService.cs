...
namespace Your.Feature.Payments.Services
{
  public class CookieIdempotencyService : IIdempotencyService
  {
    ...
    public Task<string> GetOrCreateKeyAsync(string accountNumber, string ratePlanId, string chargeId, int quantity)
    {
      ...
      if (p.k.Count >= MaxEntries) { var e = new List<string>(p.k.Keys); var drop = Math.Max(1, p.k.Count - MaxEntries + 1); for (int i=0;i<drop && i<e.Count;i++) p.k.Remove(e[i]); }
      p.k[hash] = key; Write(ctx, p, TtlSeconds);
      return Task.FromResult(key);
    }

    public Task<bool> ClearKeyAsync(string accountNumber, string ratePlanId, string chargeId, int quantity)
    {
      var ctx = HttpContext.Current; if (ctx == null) return Task.FromResult(false);
      var p = Read(ctx); if (p == null) return Task.FromResult(false);
      var hash = HashIntent(accountNumber, ratePlanId, chargeId, quantity);
      var removed = p.k.Remove(hash); Write(ctx, p, TtlSeconds);
    ...
    private static void Write(HttpContext ctx, Payload p, int ttlSeconds)
    {
      ...
      var cookie = new HttpCookie(CookieName, json){ HttpOnly=true, Secure=true, SameSite=SameSiteMode.Lax, Path="/" };
